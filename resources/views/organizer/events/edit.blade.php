...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="font-display text-2xl sm:text-3xl font-extrabold text-ink">Edit Event</h1>
            <p class="text-xs text-muted mt-1">Update event venue details, schedules, and ticket pricing tiers.</p>
        </div>
        <a href="{{ route('organizer.events.show', $event->id) }}" class="text-xs font-bold text-muted hover:text-ink transition">
            ← Cancel & Back
        </a>
    </div>

    <form action="{{ route('organizer.events.update', $event->id) }}" method="POST" class="bg-white border border-line p-6 sm:p-10 rounded-2xl space-y-6 shadow-xl shadow-black/5">
        @csrf
        @method('PUT')

        <!-- Event Status & Title Row -->
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="md:col-span-3 space-y-1.5">
                <label for="title" class="block text-xs font-bold text-ink">Event Title</label>
                <input type="text" name="title" id="title" required value="{{ old('title', $event->title) }}" placeholder="e.g. Summer Music Festival 2026"
                    class="block w-full bg-paper border border-line rounded-xl px-3.5 py-2.5 text-ink text-xs font-medium placeholder-muted transition duration-200 focus:bg-white focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent @error('title') border-rose-500 @enderror">
                @error('title') <p class="text-xs font-bold text-rose-600 mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="space-y-1.5">
                <label for="status" class="block text-xs font-bold text-ink">Status</label>
                <select name="status" id="status" class="block w-full bg-paper border border-line rounded-xl px-3.5 py-2.5 text-ink text-xs font-bold focus:bg-white focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-1.5">
                ...
                <input type="datetime-local" name="start_date" id="start_date" required value="{{ old('start_date', \Carbon\Carbon::parse($event->start_date)->format('Y-m-d\TH:i')) }}"
                    class="block w-full bg-paper border border-line rounded-xl px-3.5 py-2.5 text-ink text-xs font-medium transition duration-200 focus:bg-white focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent">
            </div>
            <div class="space-y-1.5">
                <label for="end_date" class="block text-xs font-bold text-ink">End Date & Time</label>
                <input type="datetime-local" name="end_date" id="end_date" required value="{{ old('end_date', \Carbon\Carbon::parse($event->end_date)->format('Y-m-d\TH:i')) }}"
                    class="block w-full bg-paper border border-line rounded-xl px-3.5 py-2.5 text-ink text-xs font-medium transition duration-200 focus:bg-white focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent">
            </div>
        </div>
        ...
        <div class="space-y-1.5">
            ...
            <input type="url" name="banner_url" id="banner_url" value="{{ old('banner_url', $event->banner_url) }}"
                class="block w-full bg-paper border border-line rounded-xl px-3.5 py-2.5 text-ink text-xs font-medium placeholder-muted transition duration-200 focus:bg-white focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent">
        </div>
        ...
        <div class="space-y-1.5">
            ...
            <textarea name="description" id="description" rows="4" required
                class="block w-full bg-paper border border-line rounded-xl px-3.5 py-2.5 text-ink text-xs font-medium placeholder-muted transition duration-200 focus:bg-white focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent">{{ old('description', $event->description) }}</textarea>
        </div>
        ...
        <div class="border-t border-line pt-6 space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-muted">Update pricing or add new ticket tiers.</p>
                </div>
                ...
                @foreach($event->ticketTypes as $index => $ticket)
                    <div class="ticket-row grid grid-cols-1 md:grid-cols-12 gap-3 items-end bg-paper p-4 rounded-xl border border-line">
                        ...
                        <div class="md:col-span-5 space-y-1">
                            ...
                            <input type="text" name="tickets[{{ $index }}][name]" required value="{{ old("tickets.{$index}.name", $ticket->name) }}"
                                class="block w-full bg-white border border-line rounded-lg px-3 py-2 text-xs font-medium text-ink focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent transition">
                        </div>
                        <div class="md:col-span-3 space-y-1">
                            <label class="block text-xs font-bold text-ink">Price (KES)</label>
                            <input type="number" name="tickets[{{ $index }}][price]" required min="0" step="0.01" value="{{ old("tickets.{$index}.price", $ticket->price) }}"
                                class="block w-full bg-white border border-line rounded-lg px-3 py-2 text-xs font-medium text-ink focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent transition">
                        </div>
                        <div class="md:col-span-3 space-y-1">
                            <label class="block text-xs font-bold text-ink">Total Quantity</label>
                            <input type="number" name="tickets[{{ $index }}][total_quantity]" required min="{{ $ticket->sold_quantity ?? 1 }}" value="{{ old("tickets.{$index}.total_quantity", $ticket->total_quantity) }}"
                                class="block w-full bg-white border border-line rounded-lg px-3 py-2 text-xs font-medium text-ink focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent transition">
                        </div>
                        <div class="md:col-span-1 flex justify-center pb-1">
                            <button type="button" class="remove-ticket text-rose-600 hover:text-rose-800 {{ count($event->ticketTypes) <= 1 ? 'hidden' : '' }} text-base font-bold transition">&times;</button>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="flex items-center justify-end space-x-3 pt-6 border-t border-line">
            <a href="{{ route('organizer.events.show', $event->id) }}" class="px-4 py-3 rounded-xl border border-line text-muted font-bold text-xs hover:bg-soft hover:text-ink transition">
                Cancel
            </a>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let ticketIndex = {{ count($event->ticketTypes) }};
